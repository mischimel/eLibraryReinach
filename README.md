...
- **updateBorrowed(borrowedID, borrowedDetails)**: Updates an existing borrowed item by finding it with the given borrowedID. If the borrowedID does not exist, an exception is thrown, otherwise the borrowed item is updated with the inputed values from the borrowedDetails object, if there is no input for a value, the old value is kept. The updated borrowed item is then saved. <br>
